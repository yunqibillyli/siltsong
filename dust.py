...
def mrn(grain_size_min = 5e-7, grain_size_max = 1e-4, exponent = -3.5, rho_gr = 3, sigma_rho_gr = 1.109, wavelength = 5.47e-5):
    ...
    load_Q_sca_lambda = np.vstack((np.array([[0, 0]]), np.asarray(np.loadtxt('data/scattering_efficiency.csv', delimiter = ',')).astype('float32'), np.array([[99999, 1]])))
    load_Q_ext_lambda = np.vstack((np.array([[0, 0]]), np.asarray(np.loadtxt('data/extinction_efficiency.csv', delimiter = ',')).astype('float32'), np.array([[99999, 2]])))
